...
broadcast_router.message.filter(UserRoleFilter(UserRole.ADMIN))
...
async def broadcast_handler(message, state, i18n):
    sent = await message.answer(
        text=i18n.get('broadcast'),
...
async def message_send_process(callback, bot, state, session):
    ...
    for telegram_id in telegram_ids:
        try:
            # await bot.forward_message(chat_id=telegram_id, from_chat_id=chat_id, message_id=message_id)
            await bot.copy_message(chat_id=telegram_id, from_chat_id=chat_id, message_id=message_id)
            await asyncio.sleep(0.05)
        except TelegramAPIError:
            continue

    await ser.delete_prev_messages(bot=bot, data=data, chat_id=callback.message.chat.id)  # Удаляем предыдущие сообщения
...
async def back_to_broadcast_process(callback, state, i18n):
    await callback.message.edit_text(
        text=i18n.get('broadcast'),
        reply_markup=kb.create_keyboard_broadcast()
    )
...
async def test_message_cancel_send_process(callback, bot, state):
    data = await state.get_data()
    await ser.delete_prev_messages(bot=bot, data=data, chat_id=callback.message.chat.id)  # Удаляем предыдущие сообщения
...
async def cancel_process(callback, state):
    await callback.message.edit_text(text="Вы вышли из заполнения массовой рассылки",
                                             reply_markup=None
    )
    # Сбрасываем форму заполнения данных
    await state.update_data(text=None)