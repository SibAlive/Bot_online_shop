...
from bot.services import ProductService, Product, CartService, CartItem
...
def create_cart_goods(products: list[dict]):
...
async def create_text_order(i18n: dict, products: list[dict]) -> tuple | str:
    ...
    for product in products:
        ...
        total += price * qnty
        text += (f"{name}: {qnty} * {price:,.2f} рублей = {total:,.2f} рублей;\n"
                   .replace(',', ' '))
